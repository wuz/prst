{ lib, ... }: {
  nodes.auto-update.includes = [
    (
      { host, ... }:
      let
        # Single-user machines: the update job runs as that user
        updateUser = lib.head (lib.attrNames host.users);
      in
      {
            ...
            serviceConfig = {
              ...
              ProgramArguments = [
                ...
                ''
                  ...
                  # Apply — darwin-rebuild switch --flake .#${host.hostName}
                  /run/current-system/sw/bin/darwin-rebuild switch \
                    --flake "$FLAKE_DIR#${host.hostName}" >> "$LOG" 2>&1 && \
        ...
        nixos = {
          systemd.services."prst-update" = {
            ...
            script = ''
              ...
              nixos-rebuild switch --flake "$FLAKE_DIR#${host.hostName}" >> "$LOG" 2>&1 && \
